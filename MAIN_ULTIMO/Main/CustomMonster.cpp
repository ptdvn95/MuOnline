...
void RenderModelAnimation(int Preview, int ModelID)
{
	...
	else if ( pMapNumber != 1 || gModel->SkinID != 40 )
	{
		if ( (gModel->SkinID < 644 || gModel->SkinID >= 1044) && gCustomMonster.CheckCustomMonster(lpObj->ID) == 0)
		{
			if ( gModel->SkinID > 1049 )
			{
				switch ( gModel->SkinID )
				{
					case 1116:
			...
			else if ( gModel->SkinID < 168 || gModel->SkinID > 174 )
			{
LABEL_110:
		...
		else
		{
			...
			if ( gModel->AnimationID == 7 && (gModel->SkinID == 762 || gModel->SkinID == 763 || gModel->SkinID == 794) )
			{
				if ( gModel->SkinID == 794 )
				{
