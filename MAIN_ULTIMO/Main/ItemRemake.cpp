...
//-----------------------------------------------------------------------------
	int v42;
	int ModelE;
	LPVOID v397 = pGetModel(pModelThis(), *(DWORD *)(lpModel + 48));
	ModelE = (int)v397;
	...
	if (pMapNumber != 10 && !pCheckMap(pMapNumber))
	{
		...
		if (gNoShadow.GetInfoByItem(ItemID))
			{
				return;
			}


	/*		if (ItemID >= ITEM2(7, 62) && ItemID <= ITEM2(7, 72) ||
				ItemID >= ITEM2(8, 62) && ItemID <= ITEM2(8, 72) ||
				ItemID >= ITEM2(9, 62) && ItemID <= ITEM2(9, 72) ||
				ItemID >= ITEM2(10, 62) && ItemID <= ITEM2(10, 72) ||
				ItemID >= ITEM2(11, 62) && ItemID <= ITEM2(11, 72))
			{
				return;
			}*/
