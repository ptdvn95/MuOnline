...
int DrawTimeBar(int BuffIndex, float x, float y, float Width, float Height)
{
	int result = -1;

	for(int i = 0; i < MAX_EFFECT_LIST; i++)	
	{
		if(gIconBuff.UserBuff[i].Index == *(DWORD *)BuffIndex)
		{
			int TimerBuff = 100;
	
			if(gIconBuff.UserBuff[i].MaxTime > 0 && gIconBuff.UserBuff[i].Time > 0)
			{
				TimerBuff = (gIconBuff.UserBuff[i].Time * 100) / gIconBuff.UserBuff[i].MaxTime;
			}
			pSetBlend(1);
			glColor4f(0.0, 1.0, 0.0, 1.0);
			pDrawBarForm( x,  y, Width * TimerBuff / 100.0f, Height, 0.0f, 0);
			pGLSwitchBlend();
			pGLSwitch();
...
void sub_777EB0(int BuffIndex, float x, float y, float w, float h)
{
	float Height = ( 28.0f / 100) * 75;
	float Width = ( 20.0f / 100) * 75;
	...
	if ( *(DWORD *)BuffIndex > 0 && *(DWORD *)BuffIndex < 170 )
	{
		v9 = (double)(((*(DWORD *)BuffIndex - 1) % 80) % 10) * Width / RenderW;
		v10 = (double)(((*(DWORD *)BuffIndex - 1) % 80) / 10) * Height / RenderW;
		v11 = Height / RenderW;
		v12 = Width / RenderW;

		RenderBitmap((*(DWORD *)BuffIndex - 1) / 80 + 31725, x, y, Width, Height, v9, v10, v12, v11, 1, 1, 0.0);

		IndexTime = DrawTimeBar(BuffIndex, x, (y + Height), Width, 2.0f);
		glEnable(0xDE1u);
		if( pCheckMouseOver((int) x, (int) y, (int) Width, (int) Height) )
		{
			sub_777FB0(*(DWORD *)BuffIndex, IndexTime, (Width / 2.0 + x), (y + Height + 10));
		}
	}
...
__declspec(naked) void ensamblador()
{
	static DWORD Addr_JMP = 0x00777D9F;
	static DWORD Addr = 0x007A8678;
...
void BuffIcon::Init()
{
	SetCompleteHook(0xE9, 0x00777D99, &ensamblador);
	SetCompleteHook(0xE8, 0x00777DC8, &sub_777EB0);
}
