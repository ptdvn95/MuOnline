...
void DataServerProtocolCore(BYTE head,BYTE* lpMsg,int size) // OK
{
	...
	switch(head)
	{
		...
		case 0x12:
			...
			{
			case 0x00:
				gMonsterKillCount.DGMonsterKillCountRecv((SDHP_MONSTER_KILL_COUNT_RECV*)lpMsg);
				break;
		//		case 0x00:
...
void DGCharacterInfoRecv(SDHP_CHARACTER_INFO_RECV* lpMsg) // OK
{
	...
	gDefaultClassInfo.GetCharacterFreebies(lpObj, lpMsg->IsNewChar);
...
#if (LUCKYWHEEL == 1)
	gLuckyWheel.EnvioCostoWheel(lpObj->Index);
#endif

	gPanelMenu.EnvioPanelMenu(lpObj->Index);
	...
	gResetSystem.SendResetData(lpObj, false);
	...
	{
		if(gServerInfo.m_ServerMinLevel != 0 && lpObj->Level
#if (LEVEL_PLUS ==1)
			+ lpObj->LevelPlus
#endif
			< gServerInfo.m_ServerMinLevel)
		{
			lpObj->CloseCount = 6;
			lpObj->CloseType = 3;
			return;
		...
		if(gServerInfo.m_ServerMaxLevel != 0 && lpObj->Level
#if (LEVEL_PLUS ==1)
			+ lpObj->LevelPlus
#endif
		> gServerInfo.m_ServerMaxLevel)
		{
			lpObj->CloseCount = 6;
			lpObj->CloseType = 4;
			return;
...
#if (LEVEL_PLUS== 1)
	pMsg.LevelPlus = lpObj->LevelPlus;
#endif
...
//Set Coin System
void GDSetCoinSend(int aIndex, int value1, int value2, int value3, int ruud, char * LogName) // OK
{
    LPOBJ lpObj = &gObj[aIndex];
    ...
    gDataServerConnection.DataSend((BYTE*)&pMsg,pMsg.header.size);
		...
		{
			lpObj->Ruud += ruud;
			gNotice.GCNoticeSend(lpObj->Index, 1, 0, 0, 0, 0, 0, gMessage.GetMessage(732), ruud); // hay que cambiar el mensaje
		}
		...
		{
			lpObj->Ruud += ruud;
			gNotice.GCNoticeSend(lpObj->Index, 1, 0, 0, 0, 0, 0, gMessage.GetMessage(762), ruud);  // hay que cambiar el mensaje
		}
		...
		LogAdd(LOG_ALERT,"[%s][%s] WZ_SetCoin(%s) Val1: %d, Val2: %d, Val3: %d, ruud: %d",lpObj->Account,lpObj->Name,LogName,value1,value2,value3,ruud);
}
