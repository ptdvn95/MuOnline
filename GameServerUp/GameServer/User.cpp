...
void gObjCountProc() // OK
{
	...
	for (int n = 0; n < MAX_OBJECT; n++)
	{
		if (gObj[n].Connected != OBJECT_OFFLINE)
		{
			...
			else if (gObj[n].Type == OBJECT_BOT)
			{
				TotalBots++;
			}
			else
...
bool gObjIsConnectedGP(int aIndex) // OK
{
	...
	if (lpObj->Type != OBJECT_USER  && lpObj->Type != OBJECT_BOT || (lpObj->CloseCount > 0 || lpObj->MapServerMoveQuit != 0))
	{
		return 0;
	}
...
{
	if (lpObj->Type == OBJECT_USER || lpObj->Type == OBJECT_BOT)
	{
		//------------------ cosas de pet
