...
#if (LEVEL_PLUS ==1)
	int Level = (lpObj->Level + lpObj->LevelPlus);
#endif
...
#if (LEVEL_PLUS ==1)
	if(MoveInfo.MinLevel != -1 && Level < gGate.GetMoveLevel(lpObj,gGate.GetGateMap(MoveInfo.Gate),MoveInfo.MinLevel))
#else
...
#if (LEVEL_PLUS ==1)
	if(MoveInfo.MaxLevel != -1 && Level > MoveInfo.MaxLevel)
#else
