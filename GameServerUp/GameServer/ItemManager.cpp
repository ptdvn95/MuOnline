...
BYTE CItemManager::MoveItemToTradeFromInventory(LPOBJ lpObj,BYTE SourceSlot,BYTE TargetSlot,BYTE TargetFlag) // OK
{
	...
	if (lpObj->DueloEspecial == 1)
	{
		if (gTrade.HayItemEnTrade(lpObj) == 1)
		{
			return 0xFF;
		}
	}
...
void CItemManager::CGItemBuyRecv(PMSG_ITEM_BUY_RECV* lpMsg,int aIndex,int ok) // OK
{
	...
	if(item.m_Index == GET_ITEM(13,4) || item.m_Index == GET_ITEM(13,5)) //Dark Horse,Dark Reaven
	{
		...
		{
			GDSetCoinSend(lpObj->Index, -(item.Coin1), -(item.Coin2), -(item.Coin3), 0, "CommonShop");
		}
		else
		{
	...
	if(this->InventoryInsertItemStack(lpObj,&item) != 0)
	{
		if (item.Coin1 > 0 || item.Coin2 > 0 || item.Coin3 > 0)
		{
			GDSetCoinSend(lpObj->Index, -(item.Coin1*valuex), -(item.Coin2*valuex), -(item.Coin3*valuex), 0, "CommonShopBuy");
		}
		else
		{
...
void CItemManager::CGItemSellRecv(PMSG_ITEM_SELL_RECV* lpMsg,int aIndex) // OK
{
	...
	if (lpItem->Sell > 0)
	{
		if (lpItem->Coin1 > 0)
		{
			GDSetCoinSend(lpObj->Index, value, 0, 0, 0, "CommonShopSell");
		}
		else if (lpItem->Coin2 > 0)
		{
			GDSetCoinSend(lpObj->Index, 0, value, 0, 0, "CommonShopSell");
		}
		else if (lpItem->Coin3 > 0)
		{
			GDSetCoinSend(lpObj->Index, 0, 0, value, 0, "CommonShopSell");
		}
		else
		{
...
void CItemManager::CGMoveItemProc(PMSG_MOVEITEM* aRecv, short aIndex) {
	...
	else if (aRecv->tFlag == 0 && aRecv->sFlag == 0) //equip/unequip item
	{
		...
		{ //unequipment case
			...
			// Allow some pets to move to Icarus (Uniria, Dinorant, Fenrir)
			// if(lpObj->Map == MAP_ICARUS && (lpObj->Inventory[aRecv->Source].m_Index == GET_ITEM(13,2) 
