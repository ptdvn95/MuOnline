...
bool gObjSetMonster(int aIndex,int MonsterClass) // OK
{
	...
	if(lpObj->AttackType != 0)
	{
		...
		CUSTOM_MONSTER_SKILL* MonsterAttackSkill = gCustomMonsterSkill.GetSkillMonster(MonsterClass);	//<- aqu� custommonsterskill
...
void gObjMonsterAttack(LPOBJ lpObj,LPOBJ lpTarget) // OK
{
	...
	else if(lpObj->Class == 561 || gCustomMonsterSkill.GetSkillMonster(lpObj->Class))	//<- aqu� custommonsterskill
	{
		PMSG_DURATION_SKILL_ATTACK_RECV pMsg;
