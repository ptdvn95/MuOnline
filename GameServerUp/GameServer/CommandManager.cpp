...
bool CCommandManager::ManagementCore(LPOBJ lpObj,char* message, int Npc) // OK
{
	...
	if(lpObj->Level
		...
		+ lpObj->LevelPlus
#endif
		< (CommandData.MinLevel[lpObj->AccountLevel])) //OK
	{
	...
	if(CommandData.MaxLevel[lpObj->AccountLevel] != 0 && lpObj->Level
#if (LEVEL_PLUS ==1)
		+ lpObj->LevelPlus
#endif
	> (CommandData.MaxLevel[lpObj->AccountLevel])) //OK
	{
...
//		+ lpObj->LevelPlus
#endif
		+lpObj->MasterLevel)) < gResetTable.GetResetLevel(lpObj))
	{
...
bool CCommandManager::CommandMasterReset(LPOBJ lpObj,char* arg,int Npc) // OK
{
	...
	if(((gMasterSkillTree.CheckMasterLevel(lpObj)==0)?lpObj->Level
		...
		+ lpObj->LevelPlus
#endif
		+lpObj->MasterLevel)) < gServerInfo.m_CommandMasterResetLevel[lpObj->AccountLevel])
	{
...
void CCommandManager::CommandAddPointAutoProc(LPOBJ lpObj) // OK
{
	...
	for (int n = 0; n < 5; n++)
	{
		if (lpObj->AutoAddPointStats[n] > 0)
		{
			...
			else if (stat[n] == &lpObj->Strength)
			{
				AddStat = ((((*stat[n]) + AddStat)>gServerInfo.STR_MaxStatPoint[lpObj->Class]) ? (gServerInfo.STR_MaxStatPoint[lpObj->Class] - (*stat[n])) : AddStat);
			}
			else if (stat[n] == &lpObj->Dexterity)
			{
				AddStat = ((((*stat[n]) + AddStat)>gServerInfo.AGI_MaxStatPoint[lpObj->Class]) ? (gServerInfo.AGI_MaxStatPoint[lpObj->Class] - (*stat[n])) : AddStat);
			}
			else if (stat[n] == &lpObj->Vitality)
			{
				AddStat = ((((*stat[n]) + AddStat)>gServerInfo.VIT_MaxStatPoint[lpObj->Class]) ? (gServerInfo.VIT_MaxStatPoint[lpObj->Class] - (*stat[n])) : AddStat);
			}
			else if (stat[n] == &lpObj->Energy)
			{
				AddStat = ((((*stat[n]) + AddStat)>gServerInfo.ENE_MaxStatPoint[lpObj->Class]) ? (gServerInfo.ENE_MaxStatPoint[lpObj->Class] - (*stat[n])) : AddStat);
			}
			...
			{
				lpObj->AutoAddPointStats[n] -= (((*stat[n]) >= gServerInfo.STR_MaxStatPoint[lpObj->Class]) ? lpObj->AutoAddPointStats[n] : AddStat);
			}
			else if (stat[n] == &lpObj->Dexterity)
			{
				lpObj->AutoAddPointStats[n] -= (((*stat[n]) >= gServerInfo.AGI_MaxStatPoint[lpObj->Class]) ? lpObj->AutoAddPointStats[n] : AddStat);
			}
			else if (stat[n] == &lpObj->Vitality)
			{
				lpObj->AutoAddPointStats[n] -= (((*stat[n]) >= gServerInfo.VIT_MaxStatPoint[lpObj->Class]) ? lpObj->AutoAddPointStats[n] : AddStat);
			}
			else if (stat[n] == &lpObj->Energy)
			{
				lpObj->AutoAddPointStats[n] -= (((*stat[n]) >= gServerInfo.ENE_MaxStatPoint[lpObj->Class]) ? lpObj->AutoAddPointStats[n] : AddStat);
			}
...
void CCommandManager::DGCommandMarryRecv(SDHP_COMMAND_MARRY_RECV* lpMsg) // OK
{
    ...
    else if(strcmp(lpMsg->mode,"track") == 0)
    {
        ...
        if (lpObj->Level
			...
			+ lpObj->LevelPlus
#endif
			>= gServerInfo.m_CommandMarryLevel)
		{
            if(lpTarget1 == 0)
            {
