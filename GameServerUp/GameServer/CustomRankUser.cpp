...
//	}
	...
	pMsg.iVipNivel = gObj[aTargetIndex].AccountLevel;
	pMsg.iPKLevel = gObj[aTargetIndex].PKLevel;
	...
	// ---
#if (LEVEL_PLUS== 1)
	pMsg.iLevelPlus = gObj[aTargetIndex].LevelPlus;
	pMsg.iLevelUpPoint = gObj[aTargetIndex].LevelUpPoint;
#endif
	// ---
...
int CCustomRankUser::GetRankIndex(int aIndex)
{
	...
	{
		Valor = (gObj[aIndex].Level
#if (LEVEL_PLUS ==1)
			+ gObj[aIndex].LevelPlus
#endif
			+ gObj[aIndex].MasterLevel);
	}
	else
	{
...
void CCustomRankUser::CheckUpdate(LPOBJ lpObj)
{
	...
	for(int i = 0; i < this->m_count; i++)
	{
		if(this->m_Data[i].m_iResetMin == Valor)
		{
			...
			//-- Reward
			GDSetCoinSend(lpObj->Index, this->m_Data[i].m_Coin1, this->m_Data[i].m_Coin2, this->m_Data[i].m_Coin3, 0, "RankUser");
