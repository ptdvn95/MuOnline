...
void CDevilSquare::GiveUserRewardMoney(DEVIL_SQUARE_LEVEL* lpLevel) // OK
{
	for(int n=0;n < MAX_DS_USER;n++)
	{
		...
		if (lpLevel->User[n].Rank == 0)
		{
			...
			{
				//Reward
				GDSetCoinSend(lpLevel->User[n].Index, this->m_DevilSquareRewardValue[lpLevel->Level][0], this->m_DevilSquareRewardValue[lpLevel->Level][1], this->m_DevilSquareRewardValue[lpLevel->Level][2], 0, "DS");
