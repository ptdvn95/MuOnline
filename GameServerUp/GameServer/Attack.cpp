...
bool CAttack::Attack(LPOBJ lpObj,LPOBJ lpTarget,CSkill* lpSkill,bool send,BYTE flag,int damage,int count,bool combo) // OK
{
	...
	if(damage == 0)
	{
		...
		int MinDamage = (lpObj->Level
#if (LEVEL_PLUS ==1)
			+ lpObj->LevelPlus
#endif
			+lpObj->MasterLevel)/10;
...
bool CAttack::CheckPlayerTarget(LPOBJ lpObj,LPOBJ lpTarget) // OK
{
	...
	if (gServerInfo.m_BloodCastlePvp[lpObj->AccountLevel] == 0)
	{
...
bool CAttack::MissCheckPvP(LPOBJ lpObj,LPOBJ lpTarget,CSkill* lpSkill,int send,int count,BYTE* miss) // OK
{
	...
	int AttackSuccessRate = (int)(((100*(((lpObj->AttackSuccessRatePvP*10000.0f)/(lpObj->AttackSuccessRatePvP+lpTarget->DefenseSuccessRatePvP))/10000.0f))*gServerInfo.m_ShieldGaugeAttackRate)*(((lpObj->Level
#if (LEVEL_PLUS ==1)
/*		+lpObj->LevelPlus*/
#endif
		*10000.0f)/(lpObj->Level
#if (LEVEL_PLUS ==1)
/*		+lpObj->LevelPlus*/
#endif
		+lpTarget->Level
...
int CAttack::GetAttackDamage(LPOBJ lpObj,LPOBJ lpTarget,CSkill* lpSkill,WORD* effect,int TargetDefense) // OK
{
	...
	else
	{
		...
		if(lpSkill != 0)
		{
			SkillDamageMin = (lpSkill->m_DamageMin*gSkillDamage.GetDamage(lpSkill->m_index)) / 100;
			SkillDamageMax = (lpSkill->m_DamageMax*gSkillDamage.GetDamage(lpSkill->m_index)) / 100;
...
int CAttack::GetAttackDamageWizard(LPOBJ lpObj,LPOBJ lpTarget,CSkill* lpSkill,WORD* effect,int TargetDefense) // OK
{
	...
	{
		DamageMin += (lpSkill->m_DamageMin*gSkillDamage.GetDamage(lpSkill->m_index)) / 100;
		DamageMax += (lpSkill->m_DamageMax*gSkillDamage.GetDamage(lpSkill->m_index)) / 100;
...
int CAttack::GetAttackDamageFenrir(LPOBJ lpObj,LPOBJ lpTarget,CSkill* lpSkill,WORD* effect,int TargetDefense) // OK
{
	...
	int range = ((lpSkill->m_DamageMax*gSkillDamage.GetDamage(lpSkill->m_index)) / 100 - (lpSkill->m_DamageMin*gSkillDamage.GetDamage(lpSkill->m_index)) / 100);

	range = ((range<1) ? 1 : range);

	int damage = (BaseDamage + (lpSkill->m_DamageMin*gSkillDamage.GetDamage(lpSkill->m_index)) / 100) + (GetLargeRand() % range);
...
//	int damage = (BaseDamage+lpSkill->m_DamageMin)+(GetLargeRand()%range);
	...
	if((GetLargeRand()%100) < ((lpObj->CriticalDamageRate+lpObj->EffectOption.AddCriticalDamageRate)-lpTarget->ResistCriticalDamageRate))
	{
		...
		damage = (BaseDamage + (lpSkill->m_DamageMax*gSkillDamage.GetDamage(lpSkill->m_index)) / 100);
	...
	if((GetLargeRand()%100) < ((lpObj->ExcellentDamageRate+lpObj->EffectOption.AddExcellentDamageRate)-lpTarget->ResistExcellentDamageRate))
	{
		...
		damage = ((BaseDamage + (lpSkill->m_DamageMax*gSkillDamage.GetDamage(lpSkill->m_index)) / 100) * 120) / 100;
