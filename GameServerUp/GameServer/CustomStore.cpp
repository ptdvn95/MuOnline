...
void CCustomStore::OnPShopBuyItemCallbackRecv(LPOBJ lpObj,LPOBJ lpTarget,DWORD slot,DWORD WCoinC,DWORD WCoinP,DWORD GoblinPoint) // OK
{
	...
	if (PShopWCCValue > 0)
	{
		GDSetCoinSend(lpObj->Index, -(PShopWCCValue), 0, 0,0,"CustomStoreBuy");
	}
	if (PShopWCPValue > 0)
	{
		GDSetCoinSend(lpObj->Index, 0, -(PShopWCPValue), 0,0,"CustomStoreBuy");
	}
	...
	if (PShopWCCValue > 0)
	{
		GDSetCoinSend(lpTarget->Index, PShopWCCValue, 0, 0,0,"CustomStoreSell");
	}
	if (PShopWCPValue > 0)
	{
		GDSetCoinSend(lpTarget->Index, 0, PShopWCPValue, 0,0,"CustomStoreSell");
	}
