...
int CGuild::GetMaxGuildUser(LPOBJ lpObj) // OK
{
	...
	{
		MaxGuildUser = ((lpObj->Reset==0)?(((lpObj->Level
#if (LEVEL_PLUS ==1)
			+ lpObj->LevelPlus
#endif
			+lpObj->MasterLevel)/10)+(lpObj->Leadership/10)):gServerInfo.m_GuildInsertMaxUser2);
	...
	else
	{
		MaxGuildUser = ((lpObj->Reset==0)?((lpObj->Level
			...
			+ lpObj->LevelPlus
#endif
			+lpObj->MasterLevel)/10):gServerInfo.m_GuildInsertMaxUser1);
...
void CGuild::CGGuildAssignStatusRecv(PMSG_GUILD_ASSIGN_STATUS_RECV* lpMsg,int aIndex) // OK
{
	...
	if(lpMsg->status == GUILD_STATUS_BATTLE_MASTER && this->GetGuildBattleMasterCount(lpObj) >= (((lpObj->Level
#if (LEVEL_PLUS ==1)
		+ lpObj->LevelPlus
#endif
		+lpObj->MasterLevel)/200)+1))
	{
		this->GCGuildAssignStatusSend(aIndex,lpMsg->type,18,name);
		return;
	}
...
BOOL gObjGuildMasterCapacityTest(LPOBJ lpObj)
{
	if (lpObj->Level
		...
		+ lpObj->LevelPlus
#endif
		< gServerInfo.m_GuildCreateMinLevel)
	{
		return 0;
	}
	return 1;
