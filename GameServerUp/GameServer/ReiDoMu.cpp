		...
		if (this->WinnerNumber >= 0)
		{
					...
					for(int n=OBJECT_START_USER;n < MAX_OBJECT;n++)
					{
						if(gObjIsConnectedGP(n) != 0 && gObj[n].Type == OBJECT_USER)
						{
							if (strcmp(gObj[n].GuildName,lpGuildInfo->Name) == 0)
							{
								...
								GDSetCoinSend(gObj[n].Index, this->GRewardValue1, this->GRewardValue2, this->GRewardValue3, 0, "KingOfMu");
							}
						}
					}
			...
			if(this->m_RemainTimeWinner == 16)
			{
				if (this->rank1 >= 0)
				{
					...
					GDSetCoinSend(lpObj->Index, this->CRewardValue1, this->CRewardValue2, this->CRewardValue3, 0, "KingOfMu");
