...
void CHelper::MainProc() // OK
{
	...
	for(int n=OBJECT_START_USER;n < MAX_OBJECT;n++)
	{
		...
		if(lpObj->HelperDelayTime != 0)
		{
			...
			{
				if(lpObj->Money < ((DWORD)((lpObj->Level
#if (LEVEL_PLUS ==1)
					+ lpObj->LevelPlus
#endif
					+lpObj->MasterLevel)*gServerInfo.m_HelperActiveMoney[HelperStage])))
				{
					this->DisableHelper(lpObj);
					continue;
				}
				...
				this->GCHelperStartSend(lpObj->Index,((GetTickCount()-lpObj->HelperTotalTime)/60000),((lpObj->Level
#if (LEVEL_PLUS ==1)
					+ lpObj->LevelPlus
#endif
					+lpObj->MasterLevel)*gServerInfo.m_HelperActiveMoney[HelperStage]),0);
			}
		}
	}
...
void CHelper::CGHelperStartRecv(PMSG_HELPER_START_RECV* lpMsg,int aIndex) // OK
{
	...
	if(lpObj->Level
		...
		+ lpObj->LevelPlus
#endif
		< gServerInfo.m_HelperActiveLevel)
	{
		return;
	}
