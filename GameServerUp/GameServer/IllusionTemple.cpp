...
void CIllusionTemple::SetState_CLEAN(ILLUSION_TEMPLE_LEVEL* lpLevel) // OK
{
	...
	for(int n=0;n < MAX_IT_USER;n++)
	{
		...
		if(lpLevel->User[n].Team == lpLevel->WinnerTeam)
		{
			...
			{
				//Reward
				GDSetCoinSend(lpObj->Index, this->m_IllusionTempleReward[lpLevel->Level][0], this->m_IllusionTempleReward[lpLevel->Level][1], this->m_IllusionTempleReward[lpLevel->Level][2],0,"IT");
...
void CIllusionTemple::CGIllusionTempleButtonClickRecv(int aIndex) // OK
{
	...
	{
		//Reward
		GDSetCoinSend(lpUser->Index, this->m_IllusionTempleReward[level][0], this->m_IllusionTempleReward[level][1], this->m_IllusionTempleReward[level][2], 0, "IT");
