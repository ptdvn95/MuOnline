...
struct PMSG_DUEL_OK_RECV_CUSTOM
{
#if(GAMESERVER_UPDATE>=402)
	PSBMSG_HEAD header; // C1:AA:02
	BYTE flag;
...
#if (DUEL_CUSTOM_APUESTAS == 1)
	void CGTradeAntesDelDuelo(LPOBJ lpObj, LPOBJ lpTarget);
	void CGDueloDespuesDelTrade(LPOBJ lpObj, LPOBJ lpTarget);
	int gObjGetIndex(char* szId);
	void CGDuelOkRecvCUSTOM(PMSG_DUEL_OK_RECV_CUSTOM* lpMsg, int aIndex); //CUSTOM
	void CheckDuelScoreCUSTOMPerder(LPOBJ lpObj, LPOBJ lpTarget); //CUSTOM
#endif
private:
