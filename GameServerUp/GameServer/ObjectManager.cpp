...
bool CObjectManager::CharacterGameClose(int aIndex) // OK
{
	...
	if (lpObj->DuelUser > 0)
	{
		int iIndex = lpObj->DuelUser;
		LPOBJ lpWinner = &gObj[iIndex];
		gDuel.CheckDuelScoreCUSTOMPerder(lpWinner, lpObj);
	}
#endif
...
bool CObjectManager::CharacterLevelUp(LPOBJ lpObj, DWORD AddExperience, int MaxLevelUp, int ExperienceType) // OK
{
	...
	{
		if (lpObj->MasterLevel >= gServerInfo.m_MasterSkillTreeMaxLevel && lpObj->LevelPlus >= gServerInfo.Max_LevelPlus)
		{
		...
		while (true)
		{
			if (lpObj->MasterLevel <= gServerInfo.m_MasterSkillTreeMaxLevel -1)
			{
				lpObj->MasterLevel++;
...
#if (LEVEL_PLUS == 1)
			if (gServerInfo.Max_LevelPlus > 0 && lpObj->LevelPlus <= gServerInfo.Max_LevelPlus - 1)
			{
				lpObj->LevelPlus++;
...
bool CObjectManager::CharacterLevelUpPointAdd(LPOBJ lpObj, int type, int amount) // OK
{
	...
	else if (stat == &lpObj->Strength && ((*stat) + amount) > gServerInfo.STR_MaxStatPoint[lpObj->Class])
	{

		gNotice.GCNoticeSend(lpObj->Index, 1, 0, 0, 0, 0, 0, gMessage.GetMessage(268), gServerInfo.STR_MaxStatPoint[lpObj->Class]);
		return 0;
	}
	else if (stat == &lpObj->Dexterity && ((*stat) + amount) > gServerInfo.AGI_MaxStatPoint[lpObj->Class])
	{

		gNotice.GCNoticeSend(lpObj->Index, 1, 0, 0, 0, 0, 0, gMessage.GetMessage(268), gServerInfo.AGI_MaxStatPoint[lpObj->Class]);
		return 0;
	}
	else if (stat == &lpObj->Vitality && ((*stat) + amount) > gServerInfo.VIT_MaxStatPoint[lpObj->Class])
	{

		gNotice.GCNoticeSend(lpObj->Index, 1, 0, 0, 0, 0, 0, gMessage.GetMessage(268), gServerInfo.VIT_MaxStatPoint[lpObj->Class]);
		return 0;
	}
	else if (stat == &lpObj->Energy && ((*stat) + amount) > gServerInfo.ENE_MaxStatPoint[lpObj->Class])
	{

		gNotice.GCNoticeSend(lpObj->Index, 1, 0, 0, 0, 0, 0, gMessage.GetMessage(268), gServerInfo.ENE_MaxStatPoint[lpObj->Class]);
		return 0;
	}
...
void CObjectManager::CharacterCalcSD(LPOBJ lpObj) // OK
{
	...
	lpObj->MaxShield = (((value*gServerInfo.m_ShieldGaugeConstA) / 10) + (((lpObj->Level + lpObj->MasterLevel
#if (LEVEL_PLUS ==1)
		+ lpObj->LevelPlus
#endif
		...
		+ lpObj->LevelPlus
#endif
		)) / gServerInfo.m_ShieldGaugeConstB)) + (lpObj->Defense / 2);
}
...
void CObjectManager::CharacterCalcAttribute(int aIndex) // OK
{
	...
	if (lpObj->Class == CLASS_DW)
	{
		lpObj->AttackSuccessRate = ((lpObj->Level + lpObj->MasterLevel
#if (LEVEL_PLUS ==1)
			+ lpObj->LevelPlus
#endif
			)*gServerInfo.m_DWAttackSuccessRateConstA) + ((Dexterity*gServerInfo.m_DWAttackSuccessRateConstB) / gServerInfo.m_DWAttackSuccessRateConstC) + (Strength / gServerInfo.m_DWAttackSuccessRateConstD);
	}
	else if (lpObj->Class == CLASS_DK)
	{
		lpObj->AttackSuccessRate = ((lpObj->Level + lpObj->MasterLevel
			...
			+ lpObj->LevelPlus
#endif
			)*gServerInfo.m_DKAttackSuccessRateConstA) + ((Dexterity*gServerInfo.m_DKAttackSuccessRateConstB) / gServerInfo.m_DKAttackSuccessRateConstC) + (Strength / gServerInfo.m_DKAttackSuccessRateConstD);
	}
	else if (lpObj->Class == CLASS_FE)
	{
		lpObj->AttackSuccessRate = ((lpObj->Level + lpObj->MasterLevel
			...
			+ lpObj->LevelPlus
#endif
			)*gServerInfo.m_FEAttackSuccessRateConstA) + ((Dexterity*gServerInfo.m_FEAttackSuccessRateConstB) / gServerInfo.m_FEAttackSuccessRateConstC) + (Strength / gServerInfo.m_FEAttackSuccessRateConstD);
	}
	else if (lpObj->Class == CLASS_MG)
	{
		lpObj->AttackSuccessRate = ((lpObj->Level + lpObj->MasterLevel
			...
			+ lpObj->LevelPlus
#endif
			)*gServerInfo.m_MGAttackSuccessRateConstA) + ((Dexterity*gServerInfo.m_MGAttackSuccessRateConstB) / gServerInfo.m_MGAttackSuccessRateConstC) + (Strength / gServerInfo.m_MGAttackSuccessRateConstD);
	}
	else if (lpObj->Class == CLASS_DL)
	{
		lpObj->AttackSuccessRate = ((lpObj->Level + lpObj->MasterLevel
			...
			+ lpObj->LevelPlus
#endif
			)*gServerInfo.m_DLAttackSuccessRateConstA) + ((Dexterity*gServerInfo.m_DLAttackSuccessRateConstB) / gServerInfo.m_DLAttackSuccessRateConstC) + (Strength / gServerInfo.m_DLAttackSuccessRateConstD) + (Leadership / gServerInfo.m_DLAttackSuccessRateConstE);
	}
	else if (lpObj->Class == CLASS_SU)
	{
		lpObj->AttackSuccessRate = ((lpObj->Level + lpObj->MasterLevel
			...
			+ lpObj->LevelPlus
#endif
			)*gServerInfo.m_SUAttackSuccessRateConstA) + ((Dexterity*gServerInfo.m_SUAttackSuccessRateConstB) / gServerInfo.m_SUAttackSuccessRateConstC) + (Strength / gServerInfo.m_SUAttackSuccessRateConstD);
	}
	else if (lpObj->Class == CLASS_RF)
	{
		lpObj->AttackSuccessRate = ((lpObj->Level + lpObj->MasterLevel
			...
			+ lpObj->LevelPlus
#endif
			)*gServerInfo.m_RFAttackSuccessRateConstA) + ((Dexterity*gServerInfo.m_RFAttackSuccessRateConstB) / gServerInfo.m_RFAttackSuccessRateConstC) + (Strength / gServerInfo.m_RFAttackSuccessRateConstD);
	}

	if (lpObj->Class == CLASS_DW)
	{
		lpObj->AttackSuccessRatePvP = (((lpObj->Level + lpObj->MasterLevel
#if (LEVEL_PLUS ==1)
			+ lpObj->LevelPlus
#endif
			)*gServerInfo.m_DWAttackSuccessRatePvPConstA) / gServerInfo.m_DWAttackSuccessRatePvPConstB) + ((Dexterity*gServerInfo.m_DWAttackSuccessRatePvPConstC) / gServerInfo.m_DWAttackSuccessRatePvPConstD);
	}
	else if (lpObj->Class == CLASS_DK)
	{
		lpObj->AttackSuccessRatePvP = (((lpObj->Level + lpObj->MasterLevel
			...
			+ lpObj->LevelPlus
#endif
			)*gServerInfo.m_DKAttackSuccessRatePvPConstA) / gServerInfo.m_DKAttackSuccessRatePvPConstB) + ((Dexterity*gServerInfo.m_DKAttackSuccessRatePvPConstC) / gServerInfo.m_DKAttackSuccessRatePvPConstD);
	}
	else if (lpObj->Class == CLASS_FE)
	{
		lpObj->AttackSuccessRatePvP = (((lpObj->Level + lpObj->MasterLevel
			...
			+ lpObj->LevelPlus
#endif
			)*gServerInfo.m_FEAttackSuccessRatePvPConstA) / gServerInfo.m_FEAttackSuccessRatePvPConstB) + ((Dexterity*gServerInfo.m_FEAttackSuccessRatePvPConstC) / gServerInfo.m_FEAttackSuccessRatePvPConstD);
	}
	else if (lpObj->Class == CLASS_MG)
	{
		lpObj->AttackSuccessRatePvP = (((lpObj->Level + lpObj->MasterLevel
			...
			+ lpObj->LevelPlus
#endif
			)*gServerInfo.m_MGAttackSuccessRatePvPConstA) / gServerInfo.m_MGAttackSuccessRatePvPConstB) + ((Dexterity*gServerInfo.m_MGAttackSuccessRatePvPConstC) / gServerInfo.m_MGAttackSuccessRatePvPConstD);
	}
	else if (lpObj->Class == CLASS_DL)
	{
		lpObj->AttackSuccessRatePvP = (((lpObj->Level + lpObj->MasterLevel
			...
			+ lpObj->LevelPlus
#endif
			)*gServerInfo.m_DLAttackSuccessRatePvPConstA) / gServerInfo.m_DLAttackSuccessRatePvPConstB) + ((Dexterity*gServerInfo.m_DLAttackSuccessRatePvPConstC) / gServerInfo.m_DLAttackSuccessRatePvPConstD);
	}
	else if (lpObj->Class == CLASS_SU)
	{
		lpObj->AttackSuccessRatePvP = (((lpObj->Level + lpObj->MasterLevel
			...
			+ lpObj->LevelPlus
#endif
			)*gServerInfo.m_SUAttackSuccessRatePvPConstA) / gServerInfo.m_SUAttackSuccessRatePvPConstB) + ((Dexterity*gServerInfo.m_SUAttackSuccessRatePvPConstC) / gServerInfo.m_SUAttackSuccessRatePvPConstD);
	}
	else if (lpObj->Class == CLASS_RF)
	{
		lpObj->AttackSuccessRatePvP = (((lpObj->Level + lpObj->MasterLevel
			...
			+ lpObj->LevelPlus
#endif
			)*gServerInfo.m_RFAttackSuccessRatePvPConstA) / gServerInfo.m_RFAttackSuccessRatePvPConstB) + ((Dexterity*gServerInfo.m_RFAttackSuccessRatePvPConstC) / gServerInfo.m_RFAttackSuccessRatePvPConstD);
	}
	...
	if (lpObj->Class == CLASS_DW)
	{
		lpObj->DefenseSuccessRatePvP = (((lpObj->Level + lpObj->MasterLevel
#if (LEVEL_PLUS ==1)
			+ lpObj->LevelPlus
#endif
			)*gServerInfo.m_DWDefenseSuccessRatePvPConstA) / gServerInfo.m_DWDefenseSuccessRatePvPConstB) + (Dexterity / gServerInfo.m_DWDefenseSuccessRatePvPConstC);
	}
	else if (lpObj->Class == CLASS_DK)
	{
		lpObj->DefenseSuccessRatePvP = (((lpObj->Level + lpObj->MasterLevel
			...
			+ lpObj->LevelPlus
#endif
			)*gServerInfo.m_DKDefenseSuccessRatePvPConstA) / gServerInfo.m_DKDefenseSuccessRatePvPConstB) + (Dexterity / gServerInfo.m_DKDefenseSuccessRatePvPConstC);
	}
	else if (lpObj->Class == CLASS_FE)
	{
		lpObj->DefenseSuccessRatePvP = (((lpObj->Level + lpObj->MasterLevel
			...
			+ lpObj->LevelPlus
#endif
			)*gServerInfo.m_FEDefenseSuccessRatePvPConstA) / gServerInfo.m_FEDefenseSuccessRatePvPConstB) + (Dexterity / gServerInfo.m_FEDefenseSuccessRatePvPConstC);
	}
	else if (lpObj->Class == CLASS_MG)
	{
		lpObj->DefenseSuccessRatePvP = (((lpObj->Level + lpObj->MasterLevel
			...
			+ lpObj->LevelPlus
#endif
			)*gServerInfo.m_MGDefenseSuccessRatePvPConstA) / gServerInfo.m_MGDefenseSuccessRatePvPConstB) + (Dexterity / gServerInfo.m_MGDefenseSuccessRatePvPConstC);
	}
	else if (lpObj->Class == CLASS_DL)
	{
		lpObj->DefenseSuccessRatePvP = (((lpObj->Level + lpObj->MasterLevel
			...
			+ lpObj->LevelPlus
#endif
			)*gServerInfo.m_DLDefenseSuccessRatePvPConstA) / gServerInfo.m_DLDefenseSuccessRatePvPConstB) + (Dexterity / gServerInfo.m_DLDefenseSuccessRatePvPConstC);
	}
	else if (lpObj->Class == CLASS_SU)
	{
		lpObj->DefenseSuccessRatePvP = (((lpObj->Level + lpObj->MasterLevel
			...
			+ lpObj->LevelPlus
#endif
			)*gServerInfo.m_SUDefenseSuccessRatePvPConstA) / gServerInfo.m_SUDefenseSuccessRatePvPConstB) + (Dexterity / gServerInfo.m_SUDefenseSuccessRatePvPConstC);
	}
	else if (lpObj->Class == CLASS_RF)
	{
		lpObj->DefenseSuccessRatePvP = (((lpObj->Level + lpObj->MasterLevel
			...
			+ lpObj->LevelPlus
#endif
			)*gServerInfo.m_RFDefenseSuccessRatePvPConstA) / gServerInfo.m_RFDefenseSuccessRatePvPConstB) + (Dexterity / gServerInfo.m_RFDefenseSuccessRatePvPConstC);
	}
...
#if (LEVEL_PLUS== 1)
	lpObj->LevelPlus = lpMsg->LevelPlus;
#endif
	lpObj->Zen = lpMsg->Zen;
