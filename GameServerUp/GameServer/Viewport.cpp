...
void CViewport::AddViewportObjectAgro(int aIndex,int bIndex,int type) // OK
{
	...
	{
		if (type == OBJECT_USER && OBJECT_BOT)
		{
...
void CViewport::DestroyViewportPlayer1(int aIndex) // OK
{
	...
	for(int n=0;n < MAX_VIEWPORT;n++)
	{
		...
		if (lpObj->VpPlayer[n].type != OBJECT_USER && lpObj->VpPlayer[n].type != OBJECT_BOT)
		{
			continue;
		}
...
void CViewport::DestroyViewportPlayer2(int aIndex) // OK
{
	...
	for(int n=0;n < MAX_VIEWPORT;n++)
	{
		...
		if (lpObj->VpPlayer2[n].type != OBJECT_USER && lpObj->VpPlayer2[n].type != OBJECT_BOT)
		{
			continue;
		}
...
void CViewport::DestroyViewportMonster1(int aIndex) // OK
{
	...
	for(int n=0;n < MAX_VIEWPORT;n++)
	{
		...
		if (lpObj->VpPlayer[n].type != OBJECT_MONSTER && lpObj->VpPlayer[n].type != OBJECT_BOT && lpObj->VpPlayer[n].type != OBJECT_NPC)
		{
			continue;
		}
...
void CViewport::GCViewportDestroySend(int aIndex) // OK
{
	...
	for(int n=0;n < MAX_VIEWPORT;n++)
	{
		...
		if (lpObj->VpPlayer[n].type != OBJECT_USER && lpObj->VpPlayer[n].type != OBJECT_BOT && lpObj->VpPlayer[n].type != OBJECT_MONSTER && lpObj->VpPlayer[n].type != OBJECT_NPC)
		{
			continue;
		}
...
void CViewport::GCViewportGuildSend(int aIndex) // OK
{
	...
	for(int n=0;n < MAX_VIEWPORT;n++)
	{
		...
		if (lpObj->VpPlayer[n].type != OBJECT_USER && OBJECT_BOT)
		{
			continue;
		}
...
void CViewport::GCViewportSimpleUnionSend(LPOBJ lpObj) // OK
{
	...
	for(int n=0;n < MAX_VIEWPORT;n++)
	{
		if (lpObj->VpPlayer2[n].state != VIEWPORT_NONE && lpObj->VpPlayer2[n].type == OBJECT_USER && OBJECT_BOT)
		{
