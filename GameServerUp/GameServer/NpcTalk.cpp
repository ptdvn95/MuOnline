...
bool CNpcTalk::NpcTalk(LPOBJ lpNpc,LPOBJ lpObj) // OK
{
	...
	if (gCustomNpcReq.GetNpcReq(lpObj, lpNpc->Class, lpNpc->Map, lpNpc->X, lpNpc->Y) != 0)
	{
		return 1;
	}
	...
	if (gResetSystem.Dialog(lpObj, lpNpc))
	{
		return 1;
	}
...
//	}
	...
	switch(lpNpc->Class)
	{
		...
		default:
			...
			else if (lpNpc->Class == gServerInfo.m_CustomNpcTalkPriestJames){
				this->PriestJames(lpNpc, lpObj);
			}
...
void CNpcTalk::PriestJames(LPOBJ lpNpc, LPOBJ lpObj) // OK
{
...
//		lpObj->WarehouseNumber = 0;
	...
	else
	{
		...
		pMsg.header.set(0xF3, 0x0D, sizeof(pMsg));
