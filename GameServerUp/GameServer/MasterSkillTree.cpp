...
int CMasterSkillTree::GetMasterSkillValue(LPOBJ lpObj, int index) // OK
{
	...
	//	return (int)GetRoundValue(this->GetMasterSkillValue(lpMasterSkill->m_index, lpMasterSkill->m_level));
		...
		return ((int)GetRoundValue(this->GetMasterSkillValue(lpMasterSkill->m_index, lpMasterSkill->m_level))*gSkillDamage.GetDamage(index)) / 100;
	}
...
bool CMasterSkillTree::CheckMasterLevel(LPOBJ lpObj) // OK
{
	...
	{
		if (lpObj->Level >= MAX_CHARACTER_LEVEL && lpObj->ChangeUp >= 1)
		{
			return 1;
		}
		else
...
void CMasterSkillTree::CalcMasterLevelNextExperience(LPOBJ lpObj) // OK
{
	...
	{
		lpObj->MasterExperience = ((lpObj->MasterExperience < this->m_MasterLevelExperienceTable[lpObj->LevelPlus]) ? this->m_MasterLevelExperienceTable[lpObj->LevelPlus] : lpObj->MasterExperience);

		lpObj->MasterNextExperience = this->m_MasterLevelExperienceTable[((lpObj->LevelPlus >= MAX_CHARACTER_MASTER_LEVEL) ? MAX_CHARACTER_MASTER_LEVEL : (lpObj->LevelPlus + 1))];
	}
#endif
}
